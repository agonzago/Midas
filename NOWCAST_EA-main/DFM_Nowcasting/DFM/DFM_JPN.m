...
%% User inputs.
vintage = '2020-06-22'; % vintage dataset to use for estimation
country = 'Japan';         % Global macroeconomic data
...
Spec = load_spec('Spec_GL_Japan.xls');
% Parse `Spec`
...
idxSeries = strcmp('IPALL',SeriesID); t_obs = ~isnan(X(:,idxSeries));
...
idxSeries = strcmp('IPALL',SeriesID);
...
%% Plot projection of common factor onto Real income and GDP.
idxSeries = strcmp('REAL_X',SeriesID); t_obs = ~isnan(X(:,idxSeries));
figure('Name','Common Factor Projection');

subplot(2,1,1); % projection of common factor onto RINC
...
subplot(2,1,2); % projection of common factor onto NGDPR
idxSeries = strcmp('REAL_M',SeriesID); t_obs = ~isnan(X(:,idxSeries));
CommonFactor = Res.C(idxSeries,1:5)*Res.Z(:,1:5)'*Res.Wx(idxSeries)+Res.Mx(idxSeries);
plot(Time,CommonFactor,'k'); hold on;
plot(Time(t_obs), X(t_obs,idxSeries),'b'); box on;
title(SeriesName{idxSeries}); xlim(Time([1 end])); datetick('x','yyyy','keeplimits');
