...
run_weekly_nowcast <- function(as_of_date = Sys.Date(), 
  ...
  # They are sourced in run_mexico_nowcast.R
  ...
  # Combine MIDAS forecasts
  midas_combinations <- NULL
  tryCatch({
    if (length(midas_indiv) > 0) {
      log_message("Combining MIDAS forecasts...", "INFO", log_file)
      
      # Get configuration for MIDAS combination
      trim_percentile <- if (!is.null(cfg$midas_trim_percentile)) {
        cfg$midas_trim_percentile
      } else {
        0.25  # Default: remove worst 25%
      }
      
      schemes <- if (!is.null(cfg$midas_combination_schemes)) {
        cfg$midas_combination_schemes
      } else {
        c("equal", "inv_bic", "inv_rmse")
      }
      
      midas_combinations <- combine_midas_forecasts(
        midas_indiv, 
        trim_percentile = trim_percentile,
        schemes = schemes
      )
      
      if (!is.null(midas_combinations) && length(midas_combinations) > 1) {
        log_message("MIDAS combinations generated successfully", "INFO", log_file)
        
        # Add combined MIDAS forecasts to all_fcsts for reporting
        for (scheme in schemes) {
          if (scheme %in% names(midas_combinations)) {
            combo_name <- paste0("midas_combined_", scheme)
            all_fcsts[[combo_name]] <- midas_combinations[[scheme]]
            models_updated <- c(models_updated, combo_name)
          }
        }
      }
    }
    ...
    log_message(paste("MIDAS combination failed:", e$message), "WARN", log_file)
  })
  ...
  tryCatch({
    ...
    write_weekly_summary(
      ...
      midas_combinations = midas_combinations,
