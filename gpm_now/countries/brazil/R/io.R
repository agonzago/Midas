...
#' @param midas_combinations MIDAS combination results (optional)
#' @param output_path Path to output directory
write_weekly_summary <- function(as_of_date, combo, all_fcsts, news_tbl, 
                                  models_updated = c(), midas_combinations = NULL,
                                  output_path = "output/weekly_reports") {
  ...
  # Prepare MIDAS combinations summary if available
  midas_combo_summary <- NULL
  if (!is.null(midas_combinations)) {
    midas_combo_summary <- list()
    
    # Get combination schemes (exclude metadata)
    combo_schemes <- setdiff(names(midas_combinations), "metadata")
    
    for (scheme in combo_schemes) {
      combo_data <- midas_combinations[[scheme]]
      midas_combo_summary[[scheme]] <- list(
        point = if (!is.null(combo_data$point)) combo_data$point else NA,
        lo = if (!is.null(combo_data$lo)) combo_data$lo else NA,
        hi = if (!is.null(combo_data$hi)) combo_data$hi else NA,
        weights = combo_data$weights
      )
    }
    
    # Add metadata
    if ("metadata" %in% names(midas_combinations)) {
      midas_combo_summary$metadata <- midas_combinations$metadata
    }
  }
  ...
  summary <- list(
    ...
    midas_combinations = midas_combo_summary,
    ...
    individual_forecasts = lapply(names(all_fcsts), function(name) {
      ...
      list(
        ...
        bic = if (!is.null(f$bic)) f$bic else NA,
        rmse = if (!is.null(f$rmse)) f$rmse else NA
      )
    })
  )
  ...
  # Add MIDAS combination columns to CSV if available
  if (!is.null(midas_combo_summary)) {
    for (scheme in setdiff(names(midas_combo_summary), "metadata")) {
      combo_data <- midas_combo_summary[[scheme]]
      col_name <- paste0("midas_", scheme)
      summary_df[[col_name]] <- combo_data$point
      summary_df[[paste0(col_name, "_lo")]] <- combo_data$lo
      summary_df[[paste0(col_name, "_hi")]] <- combo_data$hi
    }
  }
